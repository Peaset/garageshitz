    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Kullan " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " Plakası - " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Buraya hiçbir araç park etmediniz."
            })
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Bir islem secin - " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Kontrol etmek için araçtan uzaktasın' } )  end
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Lütfen önünüzdeki aracı hareket ettirin.' } )
...
SpawnVehicle = function(vehicleProps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Bu araç otoparkta değil.' } )
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Aracını otoparktan aldın.' } )
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Aracını park ettin.' } )
			else
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Bu araç sana ait değil.' } )
			end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Bu araç şehirde mevcut değil.' } )
    end
